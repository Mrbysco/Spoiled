...
public class SpoiledValidator {
	...
	private boolean hasValidInputsAndOutputs(SpoilRecipe recipe) {
		...
		if (recipeOutput == null || recipeOutput.isEmpty()) {
			Constants.LOGGER.error("Recipe has no output. {}", recipe.getId());
			return false;
		}
		List<Ingredient> ingredients = recipe.getIngredients();
		if (ingredients == null) {
			Constants.LOGGER.error("Recipe has no input Ingredients. {}", recipe.getId());
			return false;
		}
		...
		if (inputCount == INVALID_COUNT) {
			...
			Constants.LOGGER.error("Recipe has too many inputs. {}", recipe.getId());
			return false;
		} else if (inputCount == 0 && 1 > 0) {
			Constants.LOGGER.error("Recipe has no inputs. {}", recipe.getId());
			return false;
		}
		return true;
	}
