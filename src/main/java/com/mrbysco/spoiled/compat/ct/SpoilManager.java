	...
	public void addModSpoiling(String modName, IItemStack spoilStack, int spoilTime) {
		if (ModList.get().isLoaded(modName)) {
			for (Item foundItem : ForgeRegistries.ITEMS.getValues()) {
				ResourceLocation location = ForgeRegistries.ITEMS.getKey(foundItem);
				if (foundItem != spoilStack.getInternal().getItem() && location != null && location.getNamespace().equals(modName) && foundItem.isEdible()) {
